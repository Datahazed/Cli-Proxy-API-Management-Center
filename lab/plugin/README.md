...
- **OpenCode Go Keys Page**: Management Center includes `OpenCode Go Keys` so API keys can be pasted in without editing YAML. The plugin registers with an empty key list; adding a key PATCHes `plugins.configs.opencode-go-cliproxyapi.api-keys` and reconfigures.
...
plugins:
  configs:
    opencode-go-cliproxyapi:
      ...
      # OpenCode Go API keys (empty is allowed until you add one in
      # Management Center → OpenCode Go Keys). Supports ${ENV_VAR} expansion.
      api-keys:
...
|---|---|---|---|
| `api-keys` | `[]object` | `[]` | List of API keys (`- value: "..."`). Empty is allowed. Supports `${ENV_VAR}` expansion. Duplicates and empty values are rejected. |
