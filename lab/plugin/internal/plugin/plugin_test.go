...
func TestRegisterSuccessPublishesModels(t *testing.T) {
	...
	if len(reg.Metadata.ConfigFields) != 1 ||
		reg.Metadata.ConfigFields[0].Name != "api-keys" ||
		reg.Metadata.ConfigFields[0].Type != "array" {
		t.Fatalf("config fields wrong: %+v", reg.Metadata.ConfigFields)
	}
...
func TestRegisterSucceedsWithoutKeys(t *testing.T) {
	m, f := newTestManager(catalogResponder(true, testCatalogJSON))
	t.Cleanup(func() { _, _ = m.HandleCall("plugin.shutdown", nil) })

	resp, err := m.HandleCall("plugin.register", lifecycleRequestBody(""))
	if err != nil {
		t.Fatalf("register: %v", err)
	}
	var reg registrationResult
	decodeResult(t, resp, &reg)
	if reg.Metadata.Name != pluginName || len(reg.Metadata.ConfigFields) != 1 {
		t.Fatalf("metadata wrong: %+v", reg.Metadata)
	}
	if calls := f.callsOf(pluginabi.MethodHostHTTPDo); len(calls) != 0 {
		t.Fatalf("keyless register called catalog: %d", len(calls))
	}
	var static pluginapi.ModelResponse
	decodeResult(t, mustHandle(t, m, "model.static", nil), &static)
