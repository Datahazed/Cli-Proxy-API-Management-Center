...
func TestManagementRegistration(t *testing.T) {
	...
	if len(got.Resources) != 2 ||
		got.Resources[0].Path != "/keys" || got.Resources[0].Menu != "OpenCode Go Keys" ||
		got.Resources[1].Path != "/quota" || got.Resources[1].Menu != "OpenCode Go Quota" {
...
func TestQuotaUnknownKeyAndResource(t *testing.T) {
	...
	keysPage, err := m.HandleManagement(context.Background(), pluginapi.ManagementRequest{Method: http.MethodGet, Path: "/v0/resource/plugins/" + pluginName + "/keys"})
	if err != nil || keysPage.StatusCode != 0 || keysPage.Headers.Get("Content-Type") != "text/html; charset=utf-8" || !strings.Contains(string(keysPage.Body), "OpenCode Go Keys") || strings.Contains(string(keysPage.Body), key) {
		t.Fatalf("keys resource response = %+v err=%v", keysPage, err)
	}
}
...
func TestKeysPageReadsRememberedManagementKey(t *testing.T) {
	if !strings.Contains(resources.KeysPage, "parsed.state && parsed.state.managementKey") {
		t.Fatal("keys page does not read persisted state.managementKey")
	}
}

func TestKeysPageDecodesCurrentCPAAuthStorage(t *testing.T) {
	for _, marker := range []string{"enc::v1::", "cli-proxy-api-webui::secure-storage", "TextDecoder"} {
		if !strings.Contains(resources.KeysPage, marker) {
			t.Fatalf("keys page does not decode current CPA auth storage: missing %q", marker)
		}
	}
}

func TestKeysPageAddsViaHostPluginConfig(t *testing.T) {
	page := resources.KeysPage
	for _, marker := range []string{
		`const endpoint = "/v0/management/plugins/opencode-go-cliproxyapi/config"`,
		`request("PATCH", {"api-keys": keys.concat([{value}])})`,
		`request("PATCH", {"api-keys": next})`,
		`config["api-keys"]`,
		"Add key",
		...
		if !strings.Contains(page, marker) {
			t.Fatalf("keys page missing marker %q", marker)
		}
	}
	for _, marker := range []string{"innerHTML", "setInterval"} {
		if strings.Contains(page, marker) {
			t.Fatalf("keys page contains prohibited marker %q", marker)
		}
	}
}
