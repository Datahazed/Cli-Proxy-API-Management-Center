	...
	return okEnvelope(registrationResult{
		...
		Metadata: pluginapi.Metadata{
			...
			ConfigFields: []pluginapi.ConfigField{{
				Name: "api-keys",
				Type: "array",
				Description: "OpenCode Go API keys as JSON objects [{\"value\":\"sk-...\"}]. Empty is allowed. Prefer the OpenCode Go Keys page to add or remove keys.",
			}},
		},
...
func (m *Manager) registerManagement(request []byte) ([]byte, error) {
	...
	return okEnvelope(struct {
		...
		Resources: []struct {
			...
			{Path: "/keys", Menu: "OpenCode Go Keys", Description: "Add and remove OpenCode Go API keys."},
