...
func TestRefreshSkipsWhenNoKeys(t *testing.T) {
	cfg := config.Config{CatalogURL: "https://catalog.test/models", RequestTimeout: time.Second}
	rec := &authRecorder{}
	bridge := NewHostBridge(rec.call)
	mgr := catalog.New(cfg, bridge)
	if err := refreshOnce(context.Background(), mgr, bridge, time.Second, cfg); err != nil {
		t.Fatalf("refreshOnce with no keys: %v", err)
	}
	if auths := rec.seen(); len(auths) != 0 {
		t.Fatalf("refresh auths = %v; want none", auths)
	}
}
